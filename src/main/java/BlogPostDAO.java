...
public class BlogPostDAO {
    ...
    //Tarea 2
    // Necesito agregarle un index a los enlaces individuales de cada post
    //Para que al buscarlos carguen mas rapido.
    public Document findByPermalink(String permalink) {
        //Comando: db.posts.ensureIndex({permalink:1},{unique:true});
        Document post = postsCollection.find(new Document("permalink", permalink)).first();


        return post;
    }
    //Aqui esta la query que se encarga de cargar los posts
    public List<Document> findByDateDescending(int limit) {
        //lo que debo hacer por consola es crear un index que permita realizar esta consulta mas rapido.
        //comando: db.posts.ensureIndex({date:-1});
    ...
    //tarea 3, la pagina muestra los posts por tag
        // debo indexar los tags para hacerlos cargar mas rapido.
        //comando db.posts.ensureIndex({date:-1})
